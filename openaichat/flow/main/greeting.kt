...
val Greeting = state(Parent) {
    ...
    onEntry {
        ...
        furhat.say("I was recently introduced to the world by university of Luxembourg.")
        if (furhat.askYN("Have you heard about Chat GPT?") == true) {
            furhat.say("Good, let's try it out")
        } else {
            furhat.say("Chat GPT is a so-called language model, developed by OpenAI. It can be used to generate any text, for example a conversation, based on the description of a character. ")
...
fun ChoosePersona() = state(Parent) {
    ...
    val selectedPersonas = personasWithAvailableVoice.take(5)
